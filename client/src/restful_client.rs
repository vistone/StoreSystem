...
impl RestfulClient {
    ...
    pub async fn put_bench(
        ...
        for i in 0..rounds {
            let url = format!("{}/rest_put_{}?content_type=text/plain", self.base_url, i);
    ...
    pub async fn put_single(
        ...
        let url = format!("{}/{}?content_type=text/plain", self.base_url, key);
    ...
    pub async fn get_bench(&self, rounds: usize) -> Result<(f64, f64), Box<dyn std::error::Error>> {
        ...
        for i in 0..rounds {
            let url = format!("{}/rest_get_{}?content_type=text/plain", self.base_url, i);
            let _ = self.http.post(&url).body(value.clone()).send().await;
        }

        let mut total_ms = 0.0;
        for i in 0..rounds {
            let url = format!("{}/rest_get_{}", self.base_url, i);
            let start = Instant::now();
            let resp = self.http.get(&url).send().await?;
            if resp.status().is_success() {
        ...
        for w in 0..concurrency {
            ...
            handles.push(tokio::spawn(async move {
                ...
                for i in 0..per_worker {
                    let url = format!("{}/rest_conc_{}_{}?content_type=text/plain", base_url, w, i);
                    let s = Instant::now();
