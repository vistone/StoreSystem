...
impl store_service_server::StoreService for GrpcStoreService {
    async fn put(&self, request: Request<PutRequest>) -> Result<Response<PutResponse>, Status> {
        ...
        let meta = self
            .store
            .put(req.key.clone(), Bytes::from(req.value), content_type, tags)
            .await
            .map_err(|e| Status::internal(format!("Put failed for key '{}': {}", req.key, e)))?;
    ...
    async fn get(&self, request: Request<GetRequest>) -> Result<Response<GetResponse>, Status> {
        ...
        let (value, meta) = self.store.get(&req.key).await.map_err(|e| match e {
    ...
    ) -> Result<Response<PutBatchResponse>, Status> {
        let req = request.into_inner();
            ...
            let tags =
                if item.tags.is_empty() {
                    ...
                    Some(serde_json::from_str(&item.tags).map_err(|e| {
                        Status::invalid_argument(format!("Invalid tags JSON: {}", e))
                    })?)
                };
