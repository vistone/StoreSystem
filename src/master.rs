...
pub struct MasterNode {
    ...
    worker_clients: Arc<DashMap<String, proto::worker_service_client::WorkerServiceClient<tonic::transport::Channel>>>,
    /// Worker HTTP 客户端缓存
...
impl MasterNode {
    ...
    pub fn open_with_protocol(config: MasterConfig, protocol: &str) -> Result<Self> {
        ...
        if let Ok(registrations) = store.list_workers(false) {
            for reg in registrations {
                ...
                let tags: HashMap<String, String> = serde_json::from_str(&reg.tags_json).unwrap_or_default();
                let last_heartbeat = reg.last_heartbeat.timestamp();

                workers_map.insert(reg.worker_id.clone(), WorkerInfo {
    ...
    pub async fn register_worker(&self, worker_id: &str, address: &str, weight: i32, tags: HashMap<String, String>) -> Result<()> {
        // 先写入 SQLite 持久化
        self.store.register_worker(worker_id, address, weight, &tags)?;
    ...
    pub async fn route(&self, key: &str) -> Result<WorkerInfo> {
        let workers = self.workers.read().await;
        let mut alive_workers: Vec<&WorkerInfo> = workers.values()
            .filter(|w| w.alive)
            .collect();

        if alive_workers.is_empty() {
            return Err(StoreError::InvalidArgument("没有可用的 Worker 节点".to_string()));
        }
    ...
    async fn get_worker_client(&self, address: &str) -> Result<proto::worker_service_client::WorkerServiceClient<tonic::transport::Channel>> {
        // 先从缓存查找
        ...
        self.worker_clients.insert(address.to_string(), client.clone());
        Ok(client)
    }
    ...
    fn get_http_client(&self, address: &str) -> WorkerHttpClient {
        ...
        self.http_clients.insert(address.to_string(), client.clone());
        client
    }
    ...
    pub async fn put(&self, key: String, value: Bytes, content_type: Option<String>, tags: Option<serde_json::Value>) -> Result<ObjectMeta> {
        ...
        match self.protocol.as_str() {
            "restful" => {
                ...
                client.put(&key, value, content_type.as_deref(), tags_str.as_deref()).await
            }
            "ws" => {
            ...
            _ => {
                ...
                let request = tonic::Request::new(proto::PutRequest {
                    ...
                    tags: meta.tags.as_ref().map(|t| t.to_string()).unwrap_or_default(),
                });
    ...
    pub async fn get(&self, key: &str) -> Result<(Bytes, ObjectMeta)> {
        ...
        match self.protocol.as_str() {
            ...
            _ => {
                ...
                let meta_proto = resp.meta.ok_or_else(|| StoreError::MetaNotFound(key.to_string()))?;
                ...
                let meta = ObjectMeta {
                    ...
                    updated_at: chrono::DateTime::parse_from_rfc3339(&meta_proto.updated_at)
                        .map_err(|e| StoreError::InvalidArgument(format!("时间解析失败: {}", e)))?
                        .with_timezone(&Utc),
                    content_type: if meta_proto.content_type.is_empty() { None } else { Some(meta_proto.content_type) },
                    tags: if meta_proto.tags.is_empty() { None } else { Some(serde_json::from_str(&meta_proto.tags).map_err(|e| StoreError::InvalidArgument(format!("Tags 解析失败: {}", e)))?) },
                    checksum: None,
                    storage_node: None,
                };
    ...
    pub async fn put_batch(&self, items: Vec<(String, Bytes, Option<String>, Option<serde_json::Value>)>) -> Result<Vec<ObjectMeta>> {
        // 按 Worker 分组
        let mut worker_items: HashMap<String, Vec<(String, Bytes, Option<String>, Option<serde_json::Value>)>> = HashMap::new();

        for item in items {
            let worker = self.route(&item.0).await?;
            worker_items.entry(worker.address.clone())
                .or_default()
                .push(item);
        }
        ...
        for (address, items) in worker_items {
            let metas = match self.protocol.as_str() {
                ...
                _ => {
                    ...
                    let batch_items: Vec<proto::BatchItem> = items.iter().map(|(key, value, ct, tags)| {
                        proto::BatchItem {
                            key: key.clone(),
                            value: value.to_vec(),
...
impl proto::store_service_server::StoreService for MasterStoreService {
    async fn put(&self, request: Request<proto::PutRequest>) -> std::result::Result<Response<proto::PutResponse>, Status> {
        ...
        let tags = if req.tags.is_empty() { None } else { Some(serde_json::from_str(&req.tags).map_err(|e| Status::invalid_argument(format!("Invalid tags: {}", e)))?) };

        let meta = self.master.put(req.key, value, content_type, tags)
            .await
    ...
    async fn get(&self, request: Request<proto::GetRequest>) -> std::result::Result<Response<proto::GetResponse>, Status> {
        ...
        let (value, meta) = self.master.get(&req.key)
            .await
            .map_err(|e| match e {
    ...
    async fn exists(&self, request: Request<proto::ExistsRequest>) -> std::result::Result<Response<proto::ExistsResponse>, Status> {
        ...
        let exists = self.master.exists(&req.key)
            .await
            .map_err(|e| Status::internal(e.to_string()))?;
    ...
    async fn list(&self, request: Request<proto::ListRequest>) -> std::result::Result<Response<proto::ListResponse>, Status> {
        ...
        let metas = self.master.list(&req.prefix, req.limit as usize)
            .await
            .map_err(|e| Status::internal(e.to_string()))?;

        let proto_metas = metas.into_iter().map(|m| proto::ObjectMeta {
            key: m.key,
            size: m.size,
    ...
    async fn put_batch(&self, request: Request<proto::PutBatchRequest>) -> std::result::Result<Response<proto::PutBatchResponse>, Status> {
        ...
        for item in req.items {
            ...
            let tags = if item.tags.is_empty() { None } else { Some(serde_json::from_str(&item.tags).map_err(|e| Status::invalid_argument(format!("Invalid tags: {}", e)))?) };
        ...
        let metas = self.master.put_batch(items)
            .await
            .map_err(|e| Status::internal(e.to_string()))?;
...
impl proto::master_service_server::MasterService for MasterAdminService {
    async fn register_worker(&self, request: Request<proto::RegisterWorkerRequest>) -> std::result::Result<Response<proto::RegisterWorkerResponse>, Status> {
        let req = request.into_inner();

        self.master.register_worker(&req.worker_id, &req.address, req.weight, req.tags)
            .await
            .map_err(|e| Status::internal(e.to_string()))?;
    ...
    async fn heartbeat(&self, request: Request<proto::HeartbeatRequest>) -> std::result::Result<Response<proto::HeartbeatResponse>, Status> {
        let req = request.into_inner();

        let success = self.master.heartbeat(
            &req.worker_id,
        ...
        Ok(Response::new(proto::HeartbeatResponse {
            success,
            message: if success { "OK".to_string() } else { "Worker not found".to_string() },
        }))
    }


    async fn list_workers(&self, request: Request<proto::ListWorkersRequest>) -> std::result::Result<Response<proto::ListWorkersResponse>, Status> {
        let req = request.into_inner();

        let workers = self.master.list_workers(req.only_alive).await;

        let proto_workers = workers.into_iter().map(|w| proto::WorkerInfo {
        ...
        Ok(Response::new(proto::ListWorkersResponse { workers: proto_workers }))
    }

    async fn get_route(&self, request: Request<proto::GetRouteRequest>) -> std::result::Result<Response<proto::GetRouteResponse>, Status> {
        let req = request.into_inner();

        let worker = self.master.route(&req.key)
            .await
            .map_err(|e| Status::internal(e.to_string()))?;
