...
impl QuadShardManager {
    ...
    pub fn new(config: QuadShardConfig) -> Result<Self> {
        std::fs::create_dir_all(Path::new(&config.data_dir).join(&config.data_type))?;
        Ok(Self {
            config,
            shards: DashMap::new(),
        })
    ...
    pub fn route_paths(&self, quadkey: &str, level: u32) -> (String, PathBuf, PathBuf) {
    ...
    /// 根据 quadkey + level 确定目标 DB，懒加载打开
    fn route(&self, quadkey: &str, level: u32) -> Result<Arc<QuadShard>> {
        let db_name = if level <= self.config.base_level {
            "base".to_string()
        } else if level < self.config.split_level {
            if quadkey.len() < 4 {
    ...
    pub fn put(
        ...
        let shard = self.route(quadkey, level)?;
    ...
    /// 读取对象
    pub fn get(&self, quadkey: &str, level: u32, key: &str) -> Result<(Bytes, ObjectMeta)> {
        let shard = self.route(quadkey, level)?;
        let value = shard
            .kv_store
            .get(key)?
    ...
    /// 删除对象
    pub fn delete(&self, quadkey: &str, level: u32, key: &str) -> Result<()> {
        let shard = self.route(quadkey, level)?;
    ...
    /// 检查对象是否存在
    pub fn exists(&self, quadkey: &str, level: u32, key: &str) -> Result<bool> {
        let shard = self.route(quadkey, level)?;
...
mod tests {
    ...
    fn test_route_level_leq_base() {
        ...
        let (db_name, kv_path, _meta_path) = mgr.route_paths("30211", 5);
        assert_eq!(db_name, "base");
        assert!(kv_path.to_string_lossy().contains("test/base.kv"));
    ...
    fn test_route_level_mid() {
        let mgr = QuadShardManager::new(test_config()).unwrap();
        let (db_name, kv_path, _meta_path) = mgr.route_paths("302112345678", 12);
        assert_eq!(db_name, "3021");
        assert!(kv_path.to_string_lossy().contains("test/12/3021.kv"));
    }

    #[test]
    fn test_route_level_high() {
        let mgr = QuadShardManager::new(test_config()).unwrap();
        let (db_name, kv_path, _meta_path) = mgr.route_paths("30211234567890123456", 20);
        assert_eq!(db_name, "30211234");
        assert!(kv_path.to_string_lossy().contains("test/20/30211234.kv"));
    }
    ...
    fn test_put_and_get() {
        ...
        let meta = mgr
            .put("302112345678", 12, key, value.clone(), None, None)
            .unwrap();
        assert_eq!(meta.key, key);
        assert_eq!(meta.size, 13);

        let (read_val, read_meta) = mgr.get("302112345678", 12, key).unwrap();
    ...
    fn test_put_base_level() {
        let config = test_config();
        let mgr = QuadShardManager::new(config).unwrap();
        mgr.put("30211", 5, "k1", Bytes::from("v1"), None, None)
            .unwrap();
        mgr.put("99999", 3, "k2", Bytes::from("v2"), None, None)
            .unwrap();
        // 两个不同 quadkey 但 level ≤ 8，应该在同一 base DB
        let (v1, _) = mgr.get("30211", 5, "k1").unwrap();
        let (v2, _) = mgr.get("99999", 3, "k2").unwrap();
